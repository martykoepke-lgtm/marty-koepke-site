...
const TIER_DESCRIPTIONS: Record<string, string> = {
  ...
  Hidden: 'AI engines rarely surface this subject; competitors lead.',
  'Faintly Visible': 'Some visibility, mostly indirect; clear room to grow.',
...
<section>
  ...
  <p>The paid AI Visibility Index runs 12 queries across ChatGPT, Claude, and Perplexity, measures Presence, Citation, Share-of-Voice, and Prominence, and produces the headline visibility number.</p>
</section>`;
}
...
<section>
  ...
  <footer>
    Audit ID: <code>${escapeHtml(audit.audit_id)}</code> · Generated ${escapeHtml(audit.created_at)} · Practical Informatics LLC
  </footer>
</section>`;
}
...
<main>
  ...
  <h2>Methodology and limits</h2>
  <p>Same as in each individual report. Each subject was audited under rubric <code>${escapeHtml(audits[0]?.rubric_version ?? '')}</code>. Engines: ChatGPT, Claude (Anthropic), Perplexity. Query protocol: 4 queries × 3 engines × 1 rep = 12 calls per subject.</p>
  <footer>Generated by Practical Informatics LLC · ${new Date().toISOString()}</footer>
</main>
</body>
</html>`;
}
